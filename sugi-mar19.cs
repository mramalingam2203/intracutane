...
// Finite dose - seems to work fine -- but needs full check
static void FD_richardson(out double  [,] c,out  double [] j_0, out double[] q_0,
                    out  double [] j_X, out double[] q_X,
                    int nx,int nt,double T, double X, double d,
                     double k, double cv,double A, double V)
{
    ...
    for (int kt = 2; kt < nt-1; kt++){
        c[kt,0] = k*(cv - A*q_0[kt-1]/V);
        
        j_0[kt] =  1.0; //-(d/dx)*(c[kt-1,1]-c[kt-1,0]);
        q_0[kt] = 1.0; //q_0[kt-1] + j_0[kt]*dt;

       { for(int kx=1; kx < nx-1; kx++)
           // c[kt,kx] = r*d*c[kt-1,kx-1] + (1-2*r*d)*c[kt-1,kx] + r*d*(c[kt-1,kx+1]);
            c[kt+1 ,kx] = c[kt-1,kx] +  2*r*(c[kt,kx+1]-2*c[kt,kx]+c[kt,kx-1]);
        }
...
public static void Main()
{
    ...
    int Ntime = 200000; // no. of time steps-- should be taken care of when using explicit method because the solution may diverge
...
//  IFD_explicit(out C,out J1, out Q, Nx,Ntime ,T, H, D, C0); // infinite dose - finite difference - explicit - solver 
	FD_richardson(out C,out  J1_0, out Q_0,out J1_X, out Q_X, Nx,Ntime,T, H, D, K,Cv,A,V); // finite dose - finite difference - explicit - solver
