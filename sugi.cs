...
static void First_law(out double [,] j,out  double [,] c, out double [] q, int nt, int nx,
        double h, double tmax, double k, double cv, double l, double d) {
    ...
    for (int kt =0; kt < nt; kt++){
        ...
        for (int kx=0; kx < nx; kx++){
            ...
            c[kt,kx] = k*cv*((1-x/l)- (2*summ/Math.PI));
            
            summ = 0.0;
        
            for (int ks=1; ks <10; ks++){                         
                summ  = summ + ((k*cv*d/l)*(1+(2*(Math.Pow(-1.0,ks)     
                            *Math.Exp(-Math.Pow(ks*Math.PI/l,2.0)*d*t))))); 
                };         
            
            j[kt,kx] = k*cv*(1+ 2*summ)/l;
        ...
        summ = summ +((Math.Pow(-1.0,ks)/Math.Pow(ks,2.0))
                *Math.Exp(-Math.Pow(ks*Math.PI/l,2.0)*d*kt));
        }

        
        q[kt]= k*l*cv*((d*t/l*l)-0.1667-(2/Math.Pow(Math.PI,2.0)*summ));
...
public static void Main()
{
    ...
    double K = 1e-3;//2.1410; // partition coefficient
