...
//    First_law(out J_anal,out C_anal, out Q_anal,Ntime,Nx,H,T,K,Cv,D);


    j = new double[nt,nx+1]; // j - flux(mg/cm^2/h)
    c = new double[nt,nx+1]; //  C - concentration of chemical compound (mg/cm^3)
    q = new double[nt];     // D - diffusion coefficient(cm^2/h)

    double dx = h/nx;       // discrete element in disance (cm)
    double dt = tmax/nt;    // discrete time step(h)
    double x,t,summ;        // spatial, time, summations

    /* Further quantities 

    nt -    number of time steps
    nx -    number of discrete spatial elements
    l -     total depth of skin (cm)
    tmax -  maximum time of computation (s)
    k -     partition coefficient from vehicle to skin
    cv -    chemical concentration in vehicle (mg/cm^3) 
    d -     diffusion coefficient(cm^2/h)

    */
    
    
    Console.WriteLine(String.Format("{0},{1},{2},{3},{4},{5},{6},{7}", nt, nx, h, tmax, k, cv, h, d));
    ...
    for (int kt =0; kt < nt; kt++){
        ...
        for (int kx=0; kx < nx+1 ; kx++){
            ...
            for (int ks=1; ks <10; ks++){
                summ = summ + (Math.Sin(ks*Math.PI*x/h)
                * Math.Exp(-Math.Pow(ks*Math.PI/h,2.0)*d*t) /ks);
            };
    ...
    for (int kt=0; kt < nt; kt++){
        //Console.Write("{0:F4} {1:F4} ",kt*dt,q[kt]);
        {
...
/*
// Infnt dose - finite difference - explicit
static void IFD_explicit(out double  [,] c,out  double [] j, out double[] q, 
                int nx,int nt,double T, double X, double d, double ccnx{
...
public static void Main()
{
    ...
    int Ntime = 1000; // no. of time steps-- should be taken care of when using explicit method because the solution may diverge
    int Nx = 20; // no. of spatial elemeng
    
    double H = 0.05; // skin thickness -numerical
    double T = 6.0; // maximum time of computation
    double C0 = K*Cv;

    double[,] J_anal = new double[Ntime,Nx+1]; // flux - analytical solution
    double[,] C_anal  = new double[Ntime,Nx+1]; // concentration
    double[] Q_anal  = new double[Ntime]; // concentration
